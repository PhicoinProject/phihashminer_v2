...
std::string PhiHash::getKern(std::string kernel_code, uint64_t phi_seed, kernel_t kern)
{
    ...
    std::string kernel = std::regex_replace(kernel_code, std::regex("PHIHASH_REPLACE_HEADER"), ret.str());
    ret.str(std::string());
    ...
    kernel = std::regex_replace(kernel, std::regex("PHIHASH_REPLACE_MATH"), ret.str());
    return kernel;
}
